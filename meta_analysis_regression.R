...
run_regression_analysis <- function(df, study_name) {
  ...
  full_formula <- death ~ ARC + age + sex + charlson + vaso_day1 + sofa + wbc + lact + ph + bili + crea + pf + hr_max + temp_min
  reduced_formula <- death ~ ARC + age + sex + sofa + lact
  ...
  if ("ARC" %in% names(coef(model))) {
    ...
    # 从完整数据中获取事件计数以进行报告
    events_arc <- sum(df$death[df$ARC == 1], na.rm = TRUE)
    total_arc <- sum(df$ARC == 1, na.rm = TRUE)
    events_no_arc <- sum(df$death[df$ARC == 0], na.rm = TRUE)
    total_no_arc <- sum(df$ARC == 0, na.rm = TRUE)
...
p <- forest(
  ...
  #title = "Association of ARC with Hospital Mortality (Multivariable Model)",
  theme = tm
)
...
# --- Step 6: 保存图表 ---
ggsave(
  "../figure/Figure2_ForestPlot.pdf",
  plot = p, device = "pdf", width = 12, height = 7
)

ggsave(
  "../figure/Figure2_ForestPlot.tiff",
